...
const navLinkGroups = [
  ...
  },
  {
    ...
    links: [
      ...
      { name: 'SoundCloud', url: 'http://msn.com', key: 'key4', target: '_blank' },
      { name: 'BandCamp', url: 'http://msn.com', key: 'key4', target: '_blank' },
...
const _onRenderLink = (link) => {
  return (link.url.substr(0, 1) === '/') ?
    <Link to={link.url} key={link.key} target={link.target}>{link.name}</Link> :
    <a href={link.url} key={link.key} target={link.target}>{link.name}</a>;
}
